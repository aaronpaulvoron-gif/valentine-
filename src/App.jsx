...
export default function App() {
  ...
  // --- 🔄 THE ULTIMATE RESET (THE FIX) ---
  const restartApp = () => {
    // 1. Clear the URL parameters completely
    window.history.pushState({}, "", "/");
    // 2. Reset all states to original values
    setName("");
    setMagicLink("");
    setSubmitted(false);
    setRecipientName("");
        ...
        {isResultView ? (
          <div>
            ...
            <h1 style={styles.title}>
              {resultStatus === "yes" ? "SHE/HE SAID YES! 😍" : "She/He Said No... 🥀"}
            </h1>
            <h2 style={{color: '#555', fontSize: '1.2rem'}}>{recipientName}'s Response</h2>
            <p style={styles.finalQuote}>"{quote}"</p>
            {/* RESET BUTTON */}
            <button onClick={restartApp} style={styles.backBtn}>← Create Another Proposal</button>
          </div>
        ) : !recipientName ? (
          !submitted ? (
            <>
...
const styles = {
  ...
  proposalText: { fontSize: "1.2rem", fontWeight: "bold", color: "#444", marginBottom: '5px' },
  finalQuote: { fontSize: '1.1rem', color: '#ff4d6d', margin: '15px 0', fontWeight: 'bold' },
  stepTitle: { fontWeight: 'bold', fontSize: '0.9rem', color: '#666', marginBottom: '10px' }
};