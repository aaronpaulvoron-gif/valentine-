...
export default function App() {
  ...
  const fetchStats = async () => {
    const { data } = await supabase.from("valentine_response2").select("answered_yes");
    if (data) {
      const yes = data.filter(r => r.answered_yes).length;
      const no = data.filter(r => !r.answered_yes).length;
      setStats({ yes, no });
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        {/* --- LIVE SCOREBOARD ON TOP --- */}
        {!recipientName && (
          <div style={styles.scoreboard}>
            <div style={{color: '#4caf50'}}>YES: <b>{stats.yes}</b></div>
            <div style={{color: '#f44336'}}>NO: <b>{stats.no}</b></div>
          </div>
        )}
        ...
        {isResultView ? (
          ...
          !submitted ? (
            ...
            <>
              ...
              <button onClick={restartApp} style={styles.backBtn}>← Start Over</button>
            </>
          )
        ) : (
          <>
...
const styles = {
  ...
  card: { background: "white", padding: "40px", borderRadius: "40px", boxShadow: "0 30px 60px rgba(255, 77, 109, 0.15)", textAlign: "center", maxWidth: "420px", width: "85%", position: 'relative' },
  scoreboard: { display: 'flex', justifyContent: 'space-around', background: '#f8f8f8', padding: '10px', borderRadius: '20px', marginBottom: '20px', fontSize: '0.9rem', border: '1px solid #eee' },
